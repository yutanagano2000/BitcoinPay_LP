...
useHead({
  title: () => `${t('contactForm.business.title')} - BitcoinPay`,
});
    ...
    <div class="max-w-xl lg:max-w-2xl mx-auto">
      ...
      <div class="bg-gray-800/50 rounded-xl sm:rounded-2xl p-5 sm:p-6 lg:p-8 border border-gray-700">
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div class="w-12 h-12 sm:w-14 sm:h-14 bg-amber-500/10 rounded-xl flex items-center justify-center shrink-0">
            <UIcon name="i-heroicons-building-office" class="w-6 h-6 sm:w-7 sm:h-7 text-amber-400" />
          </div>
          <div>
            <h1 class="text-xl sm:text-2xl font-bold">{{ t('contactForm.business.title') }}</h1>
            <p class="text-sm sm:text-base text-gray-400 mt-1">{{ t('contact.business.description') }}</p>
          </div>
        </div>

        <form class="space-y-4 sm:space-y-5" @submit.prevent="handleSubmit">
          <UFormField :label="t('contactForm.business.companyName')">
            <UInput
          ...
          <UFormField :label="t('contactForm.business.department')">
            <UInput
          ...
          <UFormField :label="t('contactForm.business.name')">
            <UInput
              v-model="form.name"
              size="lg"
              class="w-full"
          ...
          <UFormField :label="t('contactForm.business.email')" required>
            <UInput
              v-model="form.email"
              type="email"
              size="lg"
          ...
          <UFormField :label="t('contactForm.business.phone')">
            <UInput
              v-model="form.phone"
              type="tel"
              size="lg"
          ...
          <UFormField :label="t('contactForm.business.inquiryType')">
            <UInput
          ...
          <UFormField :label="t('contactForm.business.message')">
            <UTextarea
